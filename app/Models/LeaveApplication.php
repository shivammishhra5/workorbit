...
class LeaveApplication extends BaseModel
{
    ...
    public function createAttendanceRecords()
    {
        if ($this->status === 'approved') {
            ...
            // Get working days from company settings (same as AttendanceRecordController)
    ...
    public function updateLeaveBalance()
    {
        ...
        $leaveBalance = \App\Models\LeaveBalance::firstOrCreate(
            ...
            [
                ...
                'allocated_days'    => $this->leavePolicy->max_days_per_year ?? 0,
                'used_days'         => 0,
                'carried_forward'   => 0,
                'manual_adjustment' => 0,
                'remaining_days'    => $this->leavePolicy->max_days_per_year ?? 0,
                'created_by'        => $this->created_by,
            ]
        );
        ...
        $leaveBalance->remaining_days  = ($leaveBalance->allocated_days + $leaveBalance->carried_forward + $leaveBalance->manual_adjustment) - $leaveBalance->used_days;
        $leaveBalance->save();
    }
}