...
class PlanOrder extends Model
{
    ...
    {
        // Guard: only pending orders can be approved
        if ($this->status !== 'pending') {
            throw new \Exception(__('This plan order has already been :status.', ['status' => $this->status]));
        }

        // Guard: user must exist
        $user = $this->user;
        if (! $user) {
            throw new \Exception(__('The user for this order no longer exists.'));
        }

        // Guard: plan must exist
        $plan = Plan::find($this->plan_id);
        if (! $plan) {
            throw new \Exception(__('The plan for this order no longer exists.'));
        }

        $billingCycle = $this->billing_cycle ?? 'monthly';

        DB::transaction(function () use ($user, $plan, $billingCycle, $processedBy) {
            $this->update([
                'status' => 'approved',
                'processed_at' => now(),
