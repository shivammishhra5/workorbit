...
class EmployeeTrainingController extends Controller
{
    ...
    {
        if (Auth::user()->can('create-employee-trainings')) {
    ...
    {
        if (Auth::user()->can('view-employee-trainings')) {
            ...
            // Resolve certification URL
            $certificationUrl = null;
            if ($employeeTraining->certification) {
                $certificationUrl = check_file($employeeTraining->certification)
                    ? get_file($employeeTraining->certification)
                    : null;
            }

            // Resolve training program materials URL
            $materialsUrl = null;
            if ($employeeTraining->trainingProgram && $employeeTraining->trainingProgram->materials) {
                $materialsUrl = check_file($employeeTraining->trainingProgram->materials)
                    ? get_file($employeeTraining->trainingProgram->materials)
                    : null;
            }
    ...
    {
        if (Auth::user()->can('edit-employee-trainings')) {
            // Check if employee training belongs to current company
            if (! in_array($employeeTraining->employee->created_by, getCompanyAndUsersId())) {
    ...
    {
        if (Auth::user()->can('delete-employee-trainings')) {
    ...
    {
        if (Auth::user()->can('record-assessment-results')) {
            $validator = Validator::make($request->all(), [
            ...
            // Auto-calculate is_passed based on score vs assessment passing_score
            $isPassed = $request->score >= $assessment->passing_score;
