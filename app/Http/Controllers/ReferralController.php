...
class ReferralController extends Controller
{
    ...
    private function superAdminView($settings)
    {
        ...
        $topCompanies->transform(function ($company) {
            $company->avatar = check_file($company->avatar)
                ? get_file($company->avatar)
                : get_file('avatars/avatar.png');

            return $company;
        });

        $payoutRequests = PayoutRequest::orderBy('created_at', 'desc')
        ->paginate(10);

        $payoutRequests->transform(function ($payout) {
            $payout->load('company');
            if ($payout->company) {
                $payout->company->avatar = check_file($payout->company->avatar)
                    ? get_file($payout->company->avatar)
                    : get_file('avatars/avatar.png');
            }
            return $payout;
        });
    ...
    private function companyView($user, $settings)
    {
        ...
        $payoutRequests = PayoutRequest::with('company')->where('company_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->paginate(10);
    ...
    public function getReferredUsers(Request $request)
    {
        ...
        $referredUsers->transform(function ($u) {
