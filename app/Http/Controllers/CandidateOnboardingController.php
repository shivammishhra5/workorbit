...
class CandidateOnboardingController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-candidate-onboarding')) {
            ...
            $candidateOnboarding->transform(function($candidate){
                $candidate->buddy_employee_name = $candidate->employee->name ?? '';
                $candidate->buddy_employee_email = $candidate->employee->email ?? '';
                $candidate->buddy_employee_avatar = check_file($candidate->employee->avatar) ? get_file($candidate->employee->avatar) : get_file('avatars/avatar.png');
                return $candidate;
            });
