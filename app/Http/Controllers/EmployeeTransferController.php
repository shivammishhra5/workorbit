    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-employee-transfers')) {
            ...
            if ($request->has('search') && !empty($request->search)) {
                $query->where(function ($transferQuery) use ($request) {
                    $transferQuery->whereHas('employee', function ($q) use ($request) {
            ...
            $departments = Department::with('branch')->whereIn('created_by', getCompanyAndUsersId())
