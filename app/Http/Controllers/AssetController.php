...
class AssetController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-assets')) {
            ...
            $assets->transform(function ($asset){
                $asset->image_url = $asset->images
                    ? (check_file($asset->images) ? get_file($asset->images) : get_file('default/image-not-found.jpg'))
                    : get_file('default/image-not-found.jpg');
                    
                if ($asset?->currentAssignment?->employee) {
                    $rawAvatar = $asset?->currentAssignment?->employee->getRawOriginal('avatar');
                    $asset->currentAssignment->employee->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
    ...
    public function dashboard(Request $request)
    {
        ...
        if (Auth::user()->can('manage-any-assets')) {
            $baseAssetQuery->whereIn('created_by', getCompanyAndUsersId());
        } elseif (Auth::user()->can('manage-own-assets')) {
            $baseAssetQuery->where(function ($q) {
                $q->where('created_by', Auth::id())
            ...
            $baseAssetQuery->whereRaw('1 = 0');
        }

        // Get asset counts by status
        $assetCounts = [
            'total' => (clone $baseAssetQuery)->count(),
            'available' => (clone $baseAssetQuery)->where('status', 'available')->count(),
            'assigned' => (clone $baseAssetQuery)->where('status', 'assigned')->count(),
            'under_maintenance' => (clone $baseAssetQuery)->where('status', 'under_maintenance')->count(),
            'disposed' => (clone $baseAssetQuery)->where('status', 'disposed')->count(),
        ];
        ...
        // Get recent assignments permission-wise
        $recentAssignmentsQuery = AssetAssignment::with(['asset', 'employee']);
        if (Auth::user()->can('manage-any-assets')) {
            $recentAssignmentsQuery->whereHas('asset', function ($q) {
                $q->whereIn('created_by', getCompanyAndUsersId());
            });
        } elseif (Auth::user()->can('manage-own-assets')) {
            $recentAssignmentsQuery->where(function ($q) {
            ...
            $recentAssignmentsQuery->whereRaw('1 = 0');
        }
        ...
        $recentAssignments = $recentAssignmentsQuery
            ...
            ->take(5)
            ->get();
        $recentAssignments->transform(function ($assignment){
            if ($assignment?->employee) {
                $rawAvatar = $assignment?->employee->getRawOriginal('avatar');
                $assignment->employee->avatar = check_file($rawAvatar)
                    ? get_file($rawAvatar)
                    : get_file('avatars/avatar.png');
            }
            return $assignment;
        ...
        // Get upcoming maintenance permission-wise
        $upcomingMaintenanceQuery = AssetMaintenance::with('asset');
        if (Auth::user()->can('manage-any-assets')) {
            $upcomingMaintenanceQuery->whereHas('asset', function ($q) {
                $q->whereIn('created_by', getCompanyAndUsersId());
            });
        } elseif (Auth::user()->can('manage-own-assets')) {
            $upcomingMaintenanceQuery->whereHas('asset', function ($q) {
            ...
            $upcomingMaintenanceQuery->whereRaw('1 = 0');
        }

        $upcomingMaintenance = $upcomingMaintenanceQuery
            ->where('status', 'scheduled')
            ->orderBy('start_date', 'asc')
            ->take(5)
            ->get();

        // Get assets with expiring warranties permission-wise
        $expiringWarranties = (clone $baseAssetQuery)
        ...
        // Get asset value summary permission-wise
        $assetValueSummary = [
            'total_purchase_value' => (clone $baseAssetQuery)->sum('purchase_cost'),
            ...
            'total_depreciation' => (clone $baseAssetQuery)->sum('purchase_cost') -
            ...
            'monthly_depreciation' => AssetDepreciation::whereHas('asset', function ($q) {
                if (Auth::user()->can('manage-any-assets')) {
                    $q->whereIn('created_by', getCompanyAndUsersId());
                } elseif (Auth::user()->can('manage-own-assets')) {
                    $q->where('created_by', Auth::id())
            ...
            })->sum('current_value') * 0.05,
        ];

        // Get recent assets permission-wise
        $recentAssets = (clone $baseAssetQuery)
            ->with(['assetType', 'currentAssignment.employee'])
            ->orderBy('created_at', 'desc')
            ->take(10)
            ->get();
            
        $recentAssets->transform(function ($asset){
            $asset->image_url = $asset->images
                ? (check_file($asset->images) ? get_file($asset->images) : get_file('default/image-not-found.jpg'))
                : get_file('default/image-not-found.jpg');
        ...
        // Calculate actual monthly asset value trend based on depreciation model rules (12 months)
        $monthlyTrend = collect(range(0, 11))->map(function($i) {
            $targetDate = now()->subMonths(11 - $i)->endOfMonth();

            $depreciations = AssetDepreciation::with('asset')
                ->whereHas('asset', function ($q) use ($targetDate) {
                    if (Auth::user()->can('manage-any-assets')) {
                        $q->whereIn('created_by', getCompanyAndUsersId());
                    } elseif (Auth::user()->can('manage-own-assets')) {
                        $q->where('created_by', Auth::id())
                    ...
                    $q->where('purchase_date', '<=', $targetDate);
                })
                ->get();

            $monthValue = $depreciations->sum(function ($dep) use ($targetDate) {
                $asset = $dep->asset;
                if (!$asset || !$asset->purchase_cost || !$asset->purchase_date) {
                    return 0;
                }

                $purchaseDate = \Carbon\Carbon::parse($asset->purchase_date);
                if ($purchaseDate->gt($targetDate)) {
                    return 0;
                }

                $ageInYears = max(0, $purchaseDate->diffInDays($targetDate) / 365);
                $cost = (float) $asset->purchase_cost;
                $salvage = (float) ($dep->salvage_value ?? 0);
                $life = (int) ($dep->useful_life_years ?: 1);

                if ($dep->method === 'reducing_balance' && $salvage > 0 && $cost > 0) {
                    $rate = 1 - pow($salvage / $cost, 1 / $life);
                    $current = $cost * pow(1 - $rate, min($ageInYears, $life));
                } else {
                    $annualDep = ($cost - $salvage) / $life;
                    $current = $cost - (min($ageInYears, $life) * $annualDep);
                }
    ...
    public function show(Asset $asset)
    {
        ...
        $asset->document_url = $asset->documents
                ? (check_file($asset->documents) ? get_file($asset->documents) : null)
                : null;
        if($asset?->currentAssignment?->employee){
            $avatar = $asset?->currentAssignment?->employee?->getRawOriginal('avatar');
            $asset->currentAssignment->employee->avatar = $avatar
                ? (check_file($avatar) ? get_file($avatar) : get_file('avatars/avatar.png'))
                : null;
        }

        $asset->assignments->transform(function ($assignment) {
            if ($assignment->employee) {
                $avatar = $assignment->employee->getRawOriginal('avatar');
        
                $assignment->employee->avatar = $avatar
                    ? (check_file($avatar) ? get_file($avatar) : get_file('avatars/avatar.png'))
                    : get_file('avatars/avatar.png');
            }
