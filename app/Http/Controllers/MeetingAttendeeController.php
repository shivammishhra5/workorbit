    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-meeting-attendees')) {
            ...
            if ($request->has('search') && !empty($request->search)) {
                $query->where(function ($meetingAttendeeQuery) use ($request) {
                    $meetingAttendeeQuery->whereHas('user', function ($q) use ($request) {
            ...
            $meetingAttendees->getCollection()->transform(function ($attendee) {
                if ($attendee->user) {
