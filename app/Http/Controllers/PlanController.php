...
class PlanController extends Controller
{
    ...
    private function companyPlansView(Request $request)
    {
        ...
        // Get the user's current plan billing cycle from latest approved order
        // For trial users, default to monthly
        ...
        $plans = $dbPlans->map(function ($plan) use ($billingCycle, $user, $userPlanCycle) {
            $price = $billingCycle === 'yearly' ? $plan->yearly_price : $plan->price;
            $features = [];
            if ($plan->enable_chatgpt === 'on') {
                $features[] = 'AI Integration';
