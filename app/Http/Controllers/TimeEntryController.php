...
class TimeEntryController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-time-entries')) {
            ...
            $timeEntries = $query->get()->map(function ($entry) use ($weekStart) {
                if (isDemo()) {
                    $originalDate = \Carbon\Carbon::parse($entry->date);
                    $entry->date = \Carbon\Carbon::parse($weekStart)->addDays($originalDate->dayOfWeekIso - 1)->toDateString();
                }
                if ($entry->employee) {
