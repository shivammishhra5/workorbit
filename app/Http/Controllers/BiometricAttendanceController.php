...
class BiometricAttendanceController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-biometric-attendance')) {
            ...
            } elseif (!empty($request->selected_date)) {
                $start_date = $request->selected_date . ' 00:00:00';
                $end_date   = $request->selected_date . ' 23:59:59';
            } else {
                // Default: today
            ...
            // Build week days for the week containing selected_date or today
            $pivotDate  = !empty($request->selected_date) ? Carbon::parse($request->selected_date) : Carbon::today();
            $weekStart  = $pivotDate->copy()->startOfWeek(Carbon::MONDAY);
            $weekDays   = [];
            for ($i = 0; $i < 7; $i++) {
            ...
            $groupedAttendances = collect($attendances)
                ...
                ->map(function ($dayEntries) {
                    ...
                    $employee = Employee::with(['user', 'designation', 'department'])->where('biometric_emp_id', $firstEntry['emp_code'])->first();

                    $avatarPath = $employee && $employee->user ? $employee->user->avatar : null;
                    return [
                        'id' => $firstEntry['id'],
                        'employee_code' => $firstEntry['emp_code'],
                        'name' => $employee && $employee->user ? $employee->user->name : 'Unknown',
                        'avatar' => $avatarPath && check_file($avatarPath) ? get_file($avatarPath) : get_file('avatars/avatar.png'),
                        ...
                        'punches' => $sorted->map(function ($entry) {
                            return [
                                'time' => date('H:i:s', strtotime($entry['punch_time'])),
                                'punch_state_display' => $entry['punch_state_display'] ?? 'Unknown',
                            ];
                        })->values(),
                    ];
                });
            ...
            // Handle employee_code filter
            if ($request->has('employee_code') && !empty($request->employee_code)) {
                $query = $query->filter(function ($item) use ($request) {
                    return $item['employee_code'] === $request->employee_code;
                });
            }
            ...
            // Fetch employees with biometric_emp_id for the filter dropdown
            ...
            return Inertia::render('hr/biometric-attendance/index', [
                ...
                'filters'              => $request->all(['search', 'employee_code', 'sort_field', 'sort_direction', 'per_page', 'selected_date']),
