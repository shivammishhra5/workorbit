    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-document-acknowledgments')) {
            ...
            $statusCounts = [
                ...
                'Exempted'     => (clone $query)->where('status', 'Exempted')->count(),
            ];
            ...
            $documentAcknowledgments->getCollection()->transform(function ($acknowledgment) {
                if ($acknowledgment->user) {
                    $rawAvatar = $acknowledgment->user->getRawOriginal('avatar');
                    $acknowledgment->user->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
                        : get_file('avatars/avatar.png');
                }
                if ($acknowledgment->assignedBy) {
                    $rawAvatar = $acknowledgment->assignedBy->getRawOriginal('avatar');
                    $acknowledgment->assignedBy->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
                        : get_file('avatars/avatar.png');
                }
                // Resolve document file URL
                if ($acknowledgment->document && $acknowledgment->document->file_path) {
                    $acknowledgment->document->file_url = check_file($acknowledgment->document->file_path)
                        ? get_file($acknowledgment->document->file_path)
                        : null;
                }
