    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-candidate-assessments')) {
            ...
            $statusCounts = [
                ...
                'Pass'    => (clone $query)->where('pass_fail_status', 'Pass')->count(),
                'Fail'    => (clone $query)->where('pass_fail_status', 'Fail')->count(),
                'Pending' => (clone $query)->where('pass_fail_status', 'Pending')->count(),
            ];
            ...
            $assessments->getCollection()->transform(function($assignment) {
                if ($assignment->conductor) {
                    $rawAvatar = $assignment->conductor->getRawOriginal('avatar');
                    $assignment->conductor->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
