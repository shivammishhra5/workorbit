...
class EmployeeGoalController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-employee-goals')) {
            ...
            $statusCounts = (clone $query)->selectRaw('status, count(*) as count')->groupBy('status')->pluck('count', 'status')->toArray();
            ...
            $goals->transform(function ($goal) {
                if ($goal->employee) {
                    $rawAvatar = $goal->employee->getRawOriginal('avatar');
                    $goal->employee->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
