...
class MeetingRoomController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-meeting-rooms')) {
            ...
            // Stats — always based on ALL rooms, never affected by filters
            $baseQuery = MeetingRoom::where(function ($q) {
                if (Auth::user()->can('manage-any-meeting-rooms')) {
                    $q->whereIn('created_by', getCompanyAndUsersId());
                } elseif (Auth::user()->can('manage-own-meeting-rooms')) {
                    $q->where('created_by', Auth::id());
            ...
            $stats = [
                ...
                'physical' => (clone $baseQuery)->where('type', 'Physical')->count(),
                'virtual'  => (clone $baseQuery)->where('type', 'Virtual')->count(),
            ];
