    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-action-items')) {
            ...
            $statusCounts = [
                ...
                'Not Started' => (clone $query)->where('status', 'Not Started')->count(),
            ...
            $actionItems->getCollection()->transform(function ($item) {
                if ($item->assignee) {
                    $rawAvatar = $item->assignee->getRawOriginal('avatar');
                    $item->assignee->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
