...
class LoginHistoryController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-login-history')) {
            $query = LoginHistory::with('user:id,name,email,type,avatar')->where(function ($q) {
                if (isSaaS()) {
            ...
            $loginHistory->getCollection()->transform(function ($history) {
                $history->avatar = check_file($history->user->avatar) ? get_file($history->user->avatar) : get_file('avatars/avatar.png');
                return $history;
            });
