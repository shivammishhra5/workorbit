    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-complaints')) {
            ...
            $statusCounts = [
                ...
                'under_investigation' => (int) ($counts['under investigation'] ?? 0),
            ...
            $complaints->getCollection()->transform(function ($complaint) {
                ...
                if ($complaint->assignedUser) {
                    $rawAvatar = $complaint->assignedUser->getRawOriginal('avatar');
                    $complaint->assignedUser->avatar = check_file($rawAvatar)
                        ? get_file($rawAvatar)
                        : get_file('avatars/avatar.png');
                }
                return $complaint;
