    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-trips')) {
            ...
            if ($request->has('search') && !empty($request->search)) {
                $query->where(function ($tripQuery) use ($request) {
                    $tripQuery->whereHas('employee', function ($q) use ($request) {
            ...
            // summaryStats: unfiltered — always shows global totals for KPI cards
            $statsQuery = Trip::where(function ($q) {
                if (Auth::user()->can('manage-any-trips')) {
                    $q->whereIn('created_by', getCompanyAndUsersId());
                } elseif (Auth::user()->can('manage-own-trips')) {
                    $q->where('created_by', Auth::id())->orWhere('employee_id', Auth::id());
            ...
            // Trend data: compare current month vs last month
            $baseQuery = Trip::where(function ($q) {
                if (Auth::user()->can('manage-any-trips')) {
                    $q->whereIn('created_by', getCompanyAndUsersId());
                } elseif (Auth::user()->can('manage-own-trips')) {
                    $q->where('created_by', Auth::id())->orWhere('employee_id', Auth::id());
            ...
            $calcTrend = function ($status) use ($baseQuery, $thisMonth, $lastMonth, $lastMonthEnd) {
                $current = (clone $baseQuery)->where('status', $status)->whereDate('created_at', '>=', $thisMonth)->count();
                $previous = (clone $baseQuery)->where('status', $status)->whereDate('created_at', '>=', $lastMonth)->whereDate('created_at', '<=', $lastMonthEnd)->count();
                if ($previous == 0) return $current > 0 ? 100 : 0;
                return round((($current - $previous) / $previous) * 100);
            };
            $allCurrent = (clone $baseQuery)->whereDate('created_at', '>=', $thisMonth)->count();
            $allPrevious = (clone $baseQuery)->whereDate('created_at', '>=', $lastMonth)->whereDate('created_at', '<=', $lastMonthEnd)->count();
            $trends = [
                'all' => $allPrevious == 0 ? ($allCurrent > 0 ? 100 : 0) : round((($allCurrent - $allPrevious) / $allPrevious) * 100),
                'planned' => $calcTrend('planned'),
                'ongoing' => $calcTrend('ongoing'),
                'completed' => $calcTrend('completed'),
                'cancelled' => $calcTrend('cancelled'),
            ];
