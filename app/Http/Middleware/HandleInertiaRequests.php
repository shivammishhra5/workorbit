    ...
    public function share(Request $request): array
    {
        ...
        if ($request->is('install/*') || $request->is('update/*') || !file_exists(storage_path('installed'))) {
            ...
            if (config('app.is_demo')) {
                ...
                $globalSettings['themeMode'] = $request->cookie('themeMode', 'light');
            } else {
                // Get user's layout direction from settings table
                if ($request->user()) {
                    $userSettings = Setting::where('user_id', $request->user()->id)
                        ->whereIn('key', ['layoutDirection', 'themeMode'])
                        ->pluck('value', 'key')->toArray();
                    if (isset($userSettings['layoutDirection'])) {
                        $globalSettings['layoutDirection'] = $userSettings['layoutDirection'];
                    }
                    if (isset($userSettings['themeMode'])) {
                        $globalSettings['themeMode'] = $userSettings['themeMode'];
                    }
                }
            }
                ...
                'user' => $request->user() ? array_merge(
                    ...
                    [
                        ...
                        'plan' => isSaas() && $request->user()->type === 'company'
                            ? optional($request->user()->plan)->only(['id', 'name', 'max_employees', 'max_users','enable_chatgpt'])
                            : null,
                        'creator' => isSaas() && !in_array($request->user()->type, ['superadmin', 'company'])
                            ? (function () use ($request) {
                                $companyId = getCompanyId($request->user()->id);
                                if (!$companyId) return null;
                                $creator = \App\Models\User::with('plan')->find($companyId);
                                if (!$creator) return null;
                                return array_merge(
                                    $creator->only(['id', 'name', 'type', 'plan_is_active', 'plan_id']),
                                    ['plan' => optional($creator->plan)->only(['id', 'name', 'enable_chatgpt'])]
                                );
                            })()
                            : null,
                    ]
