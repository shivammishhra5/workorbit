...
const applyTheme = (settings: ThemeSettings) => {
    ...
    // Apply theme color — guard against undefined
    const color = (themeColor === 'custom' ? customColor : THEME_COLORS[themeColor]) || THEME_COLORS['blue'];
    document.documentElement.style.setProperty('--theme-color', color);
    document.documentElement.style.setProperty('--primary', color);
    document.documentElement.style.setProperty('--chart-1', color);
    
    // Generate a lighter/darker variant for hover states — guard against invalid hex
    try {
