  ...
  const usageColumns = [
    {
      ...
      label: t('User'),
      sortable: true,
      render: (_: any, row: CouponUsage) => (
        <div className="flex items-center gap-3">
          {row.user_avatars ? (
            <img
              src={row.user_avatars}
              alt={row.user_name}
              className="h-9 w-9 rounded-full object-cover border border-gray-200 dark:border-gray-700"
            />
          ) : (
            <UserInitials name={row.user_name} />
          )}
          <div className="flex flex-col">
            <span className="font-semibold text-gray-900 dark:text-gray-100">{row.user_name}</span>
            <span className="text-xs text-gray-500 dark:text-gray-400">{row.user_email}</span>
          </div>
        </div>
      )
    },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {coupon.minimum_spend && (
                <div>
                  ...
                  <p className="mt-1 text-base  text-gray-900 font-mono">{window.appSettings?.formatCurrency(coupon.minimum_spend) || `$${coupon.minimum_spend.toFixed(2)}`}</p>
                </div>
              )}
              <div>
              ...
              {coupon.maximum_spend && (
                <div>
                  ...
                  <p className="mt-1 text-base  text-gray-900 font-mono">{window.appSettings?.formatCurrency(coupon.maximum_spend) || `$${coupon.maximum_spend.toFixed(2)}`}</p>
                </div>
              )}
            </div>
          </CardContent>
