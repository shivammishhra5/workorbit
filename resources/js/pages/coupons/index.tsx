...
export default function CouponsPage() {
  ...
  const { auth, coupons, filters: pageFilters = {}, globalSettings, statusCounts = {} } = usePage().props as any;
  ...
  const hasActiveFilters = () => {
    return selectedType !== '_empty_' || selectedStatus !== '_empty_' || dateFrom !== undefined || dateTo !== undefined || searchTerm !== '';
  };
  ...
  const applyFilters = () => {
    ...
    if (selectedType !== '_empty_') params.type = selectedType;
    ...
    router.get(route('coupons.index'), params, { preserveState: true, preserveScroll: true });
  };
  ...
  const statusTabs = [
    ...
    { value: '1', label: t('Active'), icon: <CheckCircle2 className="h-4 w-4" />, count: statusCounts.active ?? 0 },
    { value: '0', label: t('Inactive'), icon: <XCircle className="h-4 w-4" />, count: statusCounts.inactive ?? 0 },
  ];
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              min: 0,
              step: 0.01,
              placeholder: t('e.g. 20 for 20% or 50 for $50'),
              width: '48%'
            },
            {
