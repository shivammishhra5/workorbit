...
export default function CareerPage() {
  ...
  const handleSearch = () => {
    ...
    if (userSlug) {
      router.get(route('career.index', userSlug), params, { preserveState: true , preserveScroll : true});
    } else {
      router.get('/career', params, { preserveState: true , preserveScroll : true });
    }
  };
  ...
  const [pageInitialState, setPageInitialState] = useState(true);
    ...
    useEffect(() => {
        if (!pageInitialState) handleSearch();
        setPageInitialState(false);
    }, [searchTerm, locationFilter, salaryFilter, typeFilter, vacancyFilter]);
                    ...
                    <Card key={job.id} className="hover:shadow-lg transition-shadow border border-gray-200 flex flex-col">
                      <CardContent className="p-6 flex flex-col h-full">
                        ...
                        <div className="space-y-1 mb-4">
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-700 font-medium font-mono">
                            <span>💰</span>
                            <span>
