...
import { Plus, CheckSquare, User, Calendar, AlertTriangle, Clock, LayoutGrid, CircleDot, Loader2, BadgeCheck, AlertOctagon } from 'lucide-react';
import { format } from 'date-fns';
import { useInitials } from '@/hooks/use-initials';

export default function ActionItems() {
  const { t } = useTranslation();
  const { auth, actionItems, statusCounts = {}, meetings, employees, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  }, [statusFilter, priorityFilter, assigneeFilter, meetingFilter]);
      ...
      render: (_, row) => (
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-white overflow-hidden shrink-0">
            {row.assignee?.avatar ? (
              <img src={row.assignee.avatar} alt={row.assignee?.name} className="h-full w-full object-cover" />
            ) : (
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">{row.assignee?.email}</div>
          </div>
        </div>
      )
    },
  ...
  const statusTabs = [
    ...
    { value: 'Not Started', label: t('Not Started'), icon: <CircleDot className="h-4 w-4" />, count: statusCounts['Not Started'] ?? 0 },
  ...
  return (
    <PageTemplate
      title={t("Action Items")}
      description={t("Manage tasks and follow-ups assigned from meetings.")}
    ...
    >
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Prepare Q2 Budget Report')
            },
            {
              name: 'description',
              label: t('Description'),
              type: 'textarea',
              rows: 3,
              placeholder: t('e.g. Compile all department expenses and prepare summary...')
            },
            {
              name: 'assigned_to',
              label: t('Assign To'),
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              type: 'textarea',
              rows: 3,
              placeholder: t('e.g. Completed the first phase, working on second...'),
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View actionItem={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
