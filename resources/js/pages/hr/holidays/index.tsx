...
export default function Holidays() {
  ...
  const { auth, holidays, branches, categories, years, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  }, [selectedCategory, selectedBranch, selectedYear, dateFrom, dateTo]);
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating holiday...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating holiday...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting holiday...'));
  ...
  const handleExportPdf = () => {
    const params = new URLSearchParams({
      ...(!globalSettings?.is_demo && { year: selectedYear || new Date().getFullYear().toString() }),
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        return <div className="flex items-left gap-2 whitespace-nowrap overflow-hidden text-ellipsis text-gray-500">
            {row.start_date && <Calendar className="h-4 w-4" />}
            <span>{window.appSettings?.formatDateTimeSimple(row.start_date, false) || '-'}</span>
        </div>;
      }
  ...
  return (
    <PageTemplate
      title={t("Holidays")}
      description={t("Manage public and company holidays.")}
      url="/hr/holidays"
      actions={pageActions}
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              label: t('Description'),
              type: 'textarea',
              placeholder: t('e.g. Public holiday observed nationwide...')
            },
            {
              name: 'branch_ids',
