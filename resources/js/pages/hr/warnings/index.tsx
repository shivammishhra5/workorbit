...
import { Plus, FileText, LayoutGrid, FileEdit, AlertCircle, CheckCircle2, Clock } from 'lucide-react';
import { format } from 'date-fns';
import MediaPicker from '@/components/MediaPicker';

export default function Warnings() {
  const { t } = useTranslation();
  const { auth, warnings, employees, managers, warningTypes, filters: pageFilters = {}, globalSettings, statusCounts = {} } = usePage().props as any;
  ...
  const [selectedWarningType, setSelectedWarningType] = useState(pageFilters.warning_type || '_empty_');
  const [selectedSeverity, setSelectedSeverity] = useState(pageFilters.severity || '_empty_');
  ...
  const hasActiveFilters = () => {
    return selectedEmployee !== '_empty_' ||
           selectedWarningType !== '_empty_' ||
           selectedSeverity !== '_empty_' ||
  ...
  const activeFilterCount = () => {
    return (selectedEmployee !== '_empty_' ? 1 : 0) +
           (selectedWarningType !== '_empty_' ? 1 : 0) +
           (selectedSeverity !== '_empty_' ? 1 : 0) +
  ...
  const applyFilters = () => {
    router.get(route('hr.warnings.index'), {
      ...
      warning_type: selectedWarningType !== '_empty_' ? selectedWarningType : undefined,
      severity: selectedSeverity !== '_empty_' ? selectedSeverity : undefined,
  ...
  const handleFormSubmit = (formData: any) => {
    ...
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating warning...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating warning...'));
  ...
  const handleStatusChange = (formData: any) => {
    if (!globalSettings?.is_demo) toast.loading(t('Updating warning status...'));
  ...
  const handleImprovementPlanUpdate = (formData: any) => {
    if (!globalSettings?.is_demo) toast.loading(t('Updating improvement plan...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting warning...'));
  ...
  }, [selectedEmployee, selectedWarningType, selectedSeverity, selectedStatus, dateFrom, dateTo]);
  ...
  const actions = [
    ...
    },
    {
      ...
      requiredPermission: ['approve-warnings','acknowledge-warnings'],
    },
    {
      label: t('Improvement Plan'),
      icon: 'LineChart',
  ...
  const severityOptions = [
    { value: '_empty_', label: t('All Severities'), disabled: true },
  ...
  const statusTabs = [
    ...
    { value: 'issued', label: t('Issued'), icon: <AlertCircle className="h-4 w-4" />, count: statusCounts.issued ?? 0 },
    { value: 'acknowledged', label: t('Acknowledged'), icon: <CheckCircle2 className="h-4 w-4" />, count: statusCounts.acknowledged ?? 0 },
  ...
  return (
    <PageTemplate
      title={t("Warnings")}
      description={t("Manage formal warnings issued to employees.")}
      url="/hr/warnings"
      actions={pageActions}
    ...
    >
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              type: 'select',
              required: true,
              placeholder: t('Select Warning Type'),
            ...
            },
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Attendance Policy Violation')
            },
            {
              name: 'severity',
              label: t('Severity'),
            ...
            },
            {
              ...
              label: t('Description'),
              type: 'textarea',
              placeholder: t('e.g. Details about the warning issued...')
            },
            {
              name: 'documents',
              label: t('Documents'),
            ...
            },
            {
              ...
              type: 'textarea',
              placeholder: t('e.g. Improve attendance, Meet performance targets...'),
            ...
            ...(formMode === 'edit' ? [
              ...
              },
              {
                ...
                type: 'date',
                placeholder: t('Select Acknowledgment Date'),
              ...
              },
              {
                ...
                type: 'textarea',
                placeholder: t('e.g. Employee comments or response to the warning...'),
                showWhen: (formData) => ['acknowledged', 'expired'].includes(formData.status)
              },
              {
                ...
                type: 'textarea',
                placeholder: t('e.g. Progress made towards improvement goals...'),
                showWhen: (formData) => formData.has_improvement_plan === true || formData.has_improvement_plan === 1 || formData.has_improvement_plan === '1'
              }
            ] : [])
          ],
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View warning={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
