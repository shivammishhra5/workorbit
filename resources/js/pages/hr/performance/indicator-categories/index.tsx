...
export default function PerformanceIndicatorCategories() {
  ...
  const { auth, categories, statusCounts = {}, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  const activeFilterCount = () => {
    return (selectedStatus !== '_empty_' ? 1 : 0) + (searchTerm ? 1 : 0);
  };
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating performance indicator category...'));
      ...
      router.post(route('hr.performance.indicator-categories.store'), formData, {
        ...
        onError: (errors) => {
          ...
          if (typeof errors === 'string') {
            ...
            toast.error(t('Failed to create category: {{errors}}', { errors: Object.values(errors).join(', ') }));
          }
        }
      });
    } else if (formMode === 'edit') {
      if (!globalSettings?.is_demo) toast.loading(t('Updating performance indicator category...'));
      ...
      router.put(route('hr.performance.indicator-categories.update', currentItem.id), formData, {
        ...
        onError: (errors) => {
          ...
          if (typeof errors === 'string') {
            ...
            toast.error(t('Failed to update category: {{errors}}', { errors: Object.values(errors).join(', ') }));
          }
        }
      });
    }
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting performance indicator category...'));
    ...
    router.delete(route('hr.performance.indicator-categories.destroy', currentItem.id), {
      ...
      onError: (errors) => {
        ...
        if (typeof errors === 'string') {
          ...
          toast.error(t('Failed to delete category: {{errors}}', { errors: Object.values(errors).join(', ') }));
        }
      }
    });
  };
  ...
  const handleToggleStatus = (category: any) => {
    ...
    if (!globalSettings?.is_demo) toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} category...`);
    ...
    router.put(route('hr.performance.indicator-categories.toggle-status', category.id), {}, {
      ...
      onError: (errors) => {
        ...
        if (typeof errors === 'string') {
          ...
          toast.error(t('Failed to update category status: {{errors}}', { errors: Object.values(errors).join(', ') }));
        }
      }
    });
  };
  ...
  return (
    <PageTemplate
      title={t("Indicator Categories")}
      description={t("Manage categories used to group performance indicators.")}
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
        ...
        <Pagination
          ...
          entityName={t("performance indicator categories")}
      ...
      <CrudFormModal
        ...
        formConfig={{
          fields: [
            { name: 'name', label: t('Category Name'), type: 'text', required: true, placeholder: t('e.g. Communication Skills') },
            { name: 'description', label: t('Description'), type: 'textarea', placeholder: t('e.g. Indicators related to communication and interpersonal skills...') },
            {
              name: 'status',
              label: t('Status'),
              type: 'select',
        ...
        title={
          formMode === 'create'
            ? t('Add New Performance Indicator Category')
            : t('Edit Performance Indicator Category')
        }
        mode={formMode}
      />
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View category={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
