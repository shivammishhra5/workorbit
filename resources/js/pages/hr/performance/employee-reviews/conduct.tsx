...
export default function ConductEmployeeReview() {
  ...
  const { review, indicators, globalSettings } = usePage().props as any;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!globalSettings?.is_demo) toast.loading(t('Submitting review...'));
  ...
  return (
    <PageTemplate
      ...
      description={t("Fill in and submit the performance review for this employee.")}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('Employee')}</p>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('Review Cycle')}</p>
                    <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('Review Date')}</p>
                    <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">
              ...
              <div className="space-y-8">
                {Object.entries(indicatorsByCategory).map(([category, categoryIndicators]: [string, any]) => (
                  <div key={category} className="space-y-4">
                    <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{category}</h3>
                    <div className="space-y-6">
                      {categoryIndicators.map((indicator: any, index: number) => {
                        ...
                        return (
                          <div key={indicator.id} className="border rounded-md p-4">
                            <div className="space-y-4">
                              <div>
                                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">{indicator.name}</h4>
