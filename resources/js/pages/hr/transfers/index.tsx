...
export default function Transfers() {
  ...
  const { auth, transfers, employees, branches, departments, designations, filters: pageFilters = {}, globalSettings, statusCounts = {} } = usePage().props as any;
  ...
  const hasActiveFilters = () => {
    return selectedEmployee !== '_empty_' ||
      selectedBranch !== '_empty_' ||
      selectedDepartment !== '_empty_' ||
  ...
  const activeFilterCount = () => {
    return (selectedEmployee !== '_empty_' ? 1 : 0) +
      ...
      (selectedDepartment !== '_empty_' ? 1 : 0) +
  ...
  const handleFormSubmit = (formData: any) => {
    ...
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating transfer...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating transfer...'));
  ...
  const handleApprove = (formData: any) => {
    if (!globalSettings?.is_demo) toast.loading(t('Approving transfer...'));
  ...
  const handleReject = (formData: any) => {
    if (!globalSettings?.is_demo) toast.loading(t('Rejecting transfer...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting transfer...'));
  ...
  }, [selectedEmployee, selectedBranch, selectedDepartment, selectedStatus, dateFrom, dateTo]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        if (row.from_branch_id || row.to_branch_id) {
          rows.push({ label: t('Branch'), from: row.from_branch?.name || '—', to: row.to_branch?.name || '—' });
        }
        if (row.from_department_id || row.to_department_id) {
          rows.push({ label: t('Dept'), from: row.from_department?.name || '—', to: row.to_department?.name || '—' });
        }
        if (row.from_designation_id || row.to_designation_id) {
          rows.push({ label: t('Role'), from: row.from_designation?.name || '—', to: row.to_designation?.name || '—' });
        }
          ...
          <div className="space-y-1.5">
            {rows.map((r, i) => (
              <div key={i} className="flex items-center gap-1 min-w-0">
                <span className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset bg-blue-50 text-blue-700 ring-blue-600/20">{r.label}</span>
                <span className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[150px]" title={r.from}>{r.from}</span>
                <span className="text-gray-400 dark:text-gray-500 text-xs">→</span>
                <span className="text-xs font-medium text-gray-900 dark:text-white truncate max-w-[150px]" title={r.to}>{r.to}</span>
              </div>
            ))}
          </div>
        );
  ...
  const branchOptions = [
    { value: '_empty_', label: t('All Branches'), disabled: true },
  ...
  const departmentOptions = [
    { value: '_empty_', label: t('All Departments'), disabled: true },
  ...
  return (
    <PageTemplate
      title={t("Employee Transfers")}
      description={t("Manage employee transfers between departments and branches.")}
      url="/hr/transfers"
      actions={pageActions}
    ...
    >
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              type: 'textarea',
              required: true,
              placeholder: t('e.g. Business expansion, Employee request, Restructuring...')
            },
            {
              name: 'documents',
              label: t('Documents'),
            ...
            ...(formMode === 'edit' ? [
              ...
              },
              {
                ...
                label: t('Notes'),
                type: 'textarea',
                placeholder: t('e.g. Additional notes about this transfer...')
              }
            ] : [])
          ],
          modalSize: 'lg'
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View transfer={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
