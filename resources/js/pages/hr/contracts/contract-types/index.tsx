...
import { Plus, FileText, Calendar, Clock, LayoutGrid, CheckCircle2, XCircle } from 'lucide-react';
import { format } from 'date-fns';

export default function ContractTypes() {
  const { t } = useTranslation();
  const { auth, contractTypes, statusCounts = {}, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  return (
    <PageTemplate
      title={t("Contract Types")}
      description={t("Define categories used to classify employee contracts.")}
    ...
    >
          ...
          fields: [
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Full-time Permanent')
            },
            {
              name: 'description',
              label: t('Description'),
              type: 'textarea',
              rows: 3,
              placeholder: t('e.g. Standard full-time permanent employment contract...')
            },
            {
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View contractType={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
