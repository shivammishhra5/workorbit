...
export default function EmployeeSalaries() {
  ...
  const { auth, employeeSalaries, statusCounts = {}, employees, salaryComponents, filters: pageFilters = {}, flash, globalSettings } = usePage().props as any;
  ...
  const [selectedIsActive, setSelectedIsActive] = useState(pageFilters.is_active || '_empty_');
  ...
  const hasActiveFilters = () => {
    return searchTerm !== '' || selectedEmployee !== '_empty_' || selectedIsActive !== '_empty_';
  };

  // Count active filters
  const activeFilterCount = () => {
    return (searchTerm ? 1 : 0) + (selectedEmployee !== '_empty_' ? 1 : 0) + (selectedIsActive !== '_empty_' ? 1 : 0);
  };
  ...
  const applyFilters = () => {
    router.get(route('hr.employee-salaries.index'), {
      ...
      is_active: selectedIsActive !== '_empty_' ? selectedIsActive : undefined,
  ...
  return (
    <PageTemplate
      title={t("Employee Salaries")}
      description={t("Manage salary structures and components for employees.")}
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow mb-4 border">
        <SearchAndFilterBar
          ...
          activeStatusTab={selectedIsActive}
          onStatusTabChange={setSelectedIsActive}
          filters={[
            {
              name: 'employee_id',
              label: t('Employee'),
