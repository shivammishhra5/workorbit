...
import { Plus, BarChart, Layout, FileText, Calendar } from 'lucide-react';
...
export default function Announcements() {
  ...
  const { auth, announcements, departments, branches, categories, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  }, [selectedCategory, selectedDepartment, selectedBranch, selectedStatus, selectedPriority, isFeatured, dateFrom, dateTo]);
  ...
  const handleFormSubmit = (formData: any) => {
    ...
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating announcement...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating announcement...'));
        ...
        return (
          <div>
            <div className="flex items-left gap-2 whitespace-nowrap overflow-hidden text-ellipsis text-gray-500 mb-1">
              {start && <Calendar className="h-4 w-4" />}
              <span>{window.appSettings?.formatDateTimeSimple(start, false) || '-'}</span>
            </div>
            {end && <div className="flex items-left gap-2 whitespace-nowrap overflow-hidden text-ellipsis text-gray-500">
              <Calendar className="h-4 w-4" />
              <span>{window.appSettings?.formatDateTimeSimple(end, false) || '-'}</span>
            </div>}
  ...
  return (
    <PageTemplate
      title={t("Announcements")}
      description={t("Create and manage announcements for your organization.")}
          ...
          fields: [
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Annual Company Picnic 2025')
            },
            {
              name: 'category',
              label: t('Category'),
            ...
            },
            {
              ...
              type: 'textarea',
              required: true,
              placeholder: t('e.g. Brief summary of the announcement...'),
              ...
              dependentConfig: [
                {
                  ...
                  multiple: true,
                  placeholder: t('Select Target Branches'),
                ...
                },
                {
                  ...
                  multiple: true,
                  placeholder: t('Select Target Departments'),
