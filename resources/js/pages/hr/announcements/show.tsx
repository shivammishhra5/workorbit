...
import { ArrowLeft, BarChart, Download, Edit, Trash } from 'lucide-react';
import { useState } from 'react';
import { useTranslation } from 'react-i18next';

export default function AnnouncementShow() {
    const { t } = useTranslation();
    const { auth, announcement, viewCount, totalEmployees, viewPercentage, globalSettings } = usePage().props as any;
    ...
    const handleDeleteConfirm = () => {
        if (!globalSettings?.is_demo) toast.loading(t('Deleting announcement...'));
    ...
    return (
        <PageTemplate title={t('Announcement Details')} description={t('View the full details and recipients of this announcement.')} url={`/hr/announcements/${announcement.id}`} actions={pageActions} breadcrumbs={breadcrumbs}>
            {/* Announcement header */}
            <Card className="mb-6">
                <CardHeader>
                    <div className="flex flex-wrap items-start justify-between gap-2">
                        <div>
                            ...
                            <CardDescription className="mt-2 text-sm text-gray-600 dark:text-gray-400">{announcement.description}</CardDescription>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            ...
                            {announcement.is_featured && (
                                <Badge variant="secondary" className="bg-purple-50 text-xs text-purple-700 hover:bg-purple-50">
                                    {t('Featured')}
                                </Badge>
                            )}
                            {announcement.is_high_priority && (
                                <Badge variant="secondary" className="bg-red-50 text-xs text-red-700 hover:bg-red-50">
                                    {t('High Priority')}
                                </Badge>
                            )}
                        </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-4 text-xs text-gray-600 dark:text-gray-400">
                        <div>
                            <span className="font-semibold text-gray-700 dark:text-gray-300">{t('Start Date')}:</span>{' '}
                            <span className="ml-1">
                                {announcement.start_date
                                    ? window.appSettings?.formatDateTimeSimple(announcement.start_date, false) ||
                        ...
                        {announcement.end_date && (
                            <div>
                                <span className="font-semibold text-gray-700 dark:text-gray-300">{t('End Date')}:</span>{' '}
                                <span className="ml-1">
                                    {announcement.end_date
                                        ? window.appSettings?.formatDateTimeSimple(announcement.end_date, false) ||
                        ...
                        <div>
                            <span className="font-semibold text-gray-700 dark:text-gray-300">{t('Audience')}:</span>{' '}
                            <span className="ml-1">{announcement.is_company_wide ? t('Company-wide') : t('Targeted')}</span>
                        </div>
                    </div>
                </CardHeader>
            </Card>
            ...
            <Card className="mb-6">
                <CardContent className="pt-6">
                    <div className="prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: announcement.content }} />
                </CardContent>
                {announcement.attachments && (
                    <CardFooter>
                        <Button variant="outline" onClick={handleDownloadAttachment} className="flex items-center">
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            {announcement.departments.length > 0 && (
                                <div>
                                    <h3 className="mb-3 text-sm font-semibold text-gray-900 dark:text-gray-100">{t('Departments')}</h3>
                                    <div className="flex flex-wrap gap-2">
                            ...
                            {announcement.branches.length > 0 && (
                                <div>
                                    <h3 className="mb-3 text-sm font-semibold text-gray-900 dark:text-gray-100">{t('Branches')}</h3>
                                    <div className="flex flex-wrap gap-2">
                    ...
                    <CardContent>
                        <div className="space-y-6">
                            ...
                            <div className="flex justify-center">
                                <Button variant="outline" onClick={handleViewStatistics} className="flex items-center">
