...
export default function ConvertToEmployee() {
  ...
  const handleProfileImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (errors['profile_image']) {
        setErrors(prev => { const n = { ...prev }; delete n['profile_image']; return n; });
      }
    }
  };
  ...
  return (
    <PageTemplate
      ...
      description={t("Convert a selected candidate into a new employee record.")}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-3">
                  <div className="border rounded-md p-4 flex items-center justify-center bg-muted/30 h-32">
                    {profileImagePreview ? (
                      <img src={profileImagePreview} alt="Profile Image" className="max-h-full max-w-full object-contain rounded-full" />
                    ) : (
                  ...
                  <Input type="file" accept="image/*" onChange={handleProfileImageChange} className="cursor-pointer" />
