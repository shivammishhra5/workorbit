...
import { Plus, LayoutGrid, Sparkles, ScanSearch, MessageSquare, FileText, UserCheck, UserX, Kanban } from 'lucide-react';
...
export default function Candidates() {
  ...
  const pageActions = [
    {
      ...
      onClick: () => router.get(route('hr.recruitment.candidates.kanban'))
    }
  ];
  ...
  const columns = [
    {
      ...
      render: (value: any, row: any) => {
        ...
        <div className="flex items-center gap-3">
            <UserInitials name={`${row.first_name} ${row.last_name}`} />
            <div>
  ...
  const statusTabs = [
    { value: '_empty_',   label: t('All'),        icon: <LayoutGrid className="h-4 w-4" />,    count: statusCounts.all       ?? (candidates?.total || 0) },
    { value: 'New',       label: t('New'),        icon: <Sparkles className="h-4 w-4" />,      count: statusCounts.New       ?? 0 },
    { value: 'Screening', label: t('Screening'),  icon: <ScanSearch className="h-4 w-4" />,    count: statusCounts.Screening ?? 0 },
    { value: 'Interview', label: t('Interview'),  icon: <MessageSquare className="h-4 w-4" />, count: statusCounts.Interview ?? 0 },
    { value: 'Offer',     label: t('Offer'),      icon: <FileText className="h-4 w-4" />,      count: statusCounts.Offer     ?? 0 },
    { value: 'Hired',     label: t('Hired'),      icon: <UserCheck className="h-4 w-4" />,     count: statusCounts.Hired     ?? 0 },
    { value: 'Rejected',  label: t('Rejected'),   icon: <UserX className="h-4 w-4" />,         count: statusCounts.Rejected  ?? 0 },
  ];
  ...
  }, [searchTerm, statusFilter, jobFilter, sourceFilter]);
  ...
  return (
    <PageTemplate
      title={t("Candidates")}
      description={t("View and manage candidates across all job postings.")}
