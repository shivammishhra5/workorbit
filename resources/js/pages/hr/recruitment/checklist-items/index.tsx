...
export default function ChecklistItems() {
  ...
  const { auth, checklistItems, checklists, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating checklist item...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating checklist item...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting checklist item...'));
  ...
  const handleToggleStatus = (item: any) => {
    ...
    if (!globalSettings?.is_demo) toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} checklist item...`);
  ...
  }, [searchTerm, categoryFilter, checklistFilter, requiredFilter]);
  ...
  return (
    <PageTemplate
      title={t("Checklist Items")}
      description={t("Manage individual items within onboarding checklists.")}
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Submit signed employment contract')
            },
            {
              name: 'description',
              label: t('Description'),
              type: 'textarea',
              placeholder: t('e.g. Employee must sign and return the employment contract...')
            },
            {
              name: 'category',
              label: t('Category'),
