...
export default function OnboardingChecklists() {
  ...
  const { auth, onboardingChecklists, filters: pageFilters = {}, globalSettings, statusCounts = {} } = usePage().props as any;
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating onboarding checklist...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating onboarding checklist...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting onboarding checklist...'));
  ...
  const handleToggleStatus = (item: any) => {
    ...
    if (!globalSettings?.is_demo) toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} onboarding checklist...`);
  ...
  }, [searchTerm, statusFilter, defaultFilter]);
  ...
  return (
    <PageTemplate
      title={t("Onboarding Checklists")}
      description={t("Manage onboarding checklists used for new hires.")}
    ...
    >
          ...
          fields: [
            {
              ...
              type: 'text',
              required: true,
              placeholder: t('e.g. Software Engineer Onboarding')
            },
            {
              name: 'description',
              label: t('Description'),
              type: 'textarea',
              placeholder: t('e.g. Standard onboarding checklist for new software engineers...')
            },
            {
              name: 'is_default',
      ...
      <Dialog open={!!viewingItem} onOpenChange={() => setViewingItem(null)}>
        {viewingItem && <View checklist={viewingItem} />}
      </Dialog>
    </PageTemplate>
  );
}
