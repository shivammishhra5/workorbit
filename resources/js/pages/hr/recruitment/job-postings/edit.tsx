...
export default function EditJobPosting() {
  ...
  const { jobPosting, jobTypes, locations, branches, departments, customQuestions, companySlug, globalSettings } = usePage().props as any;
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 0) {
      ...
      if (!data.status) e.status = t('Status is required');
  ...
  const STEP_FIELDS: Record<number, string[]> = {
    0: ['title', 'job_type_id', 'location_id', 'branch_id', 'department_id', 'priority', 'status', 'skills', 'start_date', 'application_deadline', 'application_type', 'application_url', 'positions'],
  ...
  const handleSubmit = () => {
    ...
    if (!globalSettings?.is_demo) toast.loading(t('Updating job posting...'));
  ...
  return (
    <PageTemplate
      ...
      description={t("Update the details of an existing job posting.")}
